...
-- GET THE MINIMUM PRICE FROMo PRODUCTS TABLE
...
USE GENAI;
-- WHAT IS SQL JOINS?
-- JOINS ARE USED TO COMBINE RECORDS FROM TWO OR MORE TABLES BASED ON REALTED COLUMNS 
-- WHY WE USE SQL JOINS?
-- RETRIVE THE RELATED DATA FROM MULTIPLE TABLES
-- NORMALIZE THE DATA BY SEPERATING IT INTO LOGICAL TABLES
-- CREATE A SAMPLE TABLE ON EMPLOYEESS

CREATE TABLE EMPLOYEE(EMP_ID INT PRIMARY KEY, EMP_NAME VARCHAR(25),DEPT_ID INT);
-- INSERT THE DATA INTO TABLES
INSERT INTO EMPLOYEE VALUES(1,'ALICE',10),(2,'BOB',20),(3,'CHARLICE',30),(4,'DAVID',NULL);
-- CHECK THE DATA FROM EMPLOYEE TABLE
SELECT * FROM EMPLOYEE;
-- CREATE TABLE DEPARTMENTS WHERE DEPT_ID AND DEPT_NAME
CREATE TABLE DEPT(DEPT_ID INT PRIMARY KEY,DEPT_NAME VARCHAR(25));
-- INSERT THE DATA INTO DEPT
INSERT INTO DEPT VALUES(10,'HR'),(20,'IT'),(30,'FINANCE'),(40,'MARKETING');
-- CHECK THE DATA FROM DEPT
SELECT * FROM DEPT;

-- THE COMMON COLUMN USED TO JOIN THE TWO TABLES IS KNOWN AS FOREIGN KEYS
-- 1. INNER JOIN - IT GETS THE MATCHED RECORDS FROM THE BOTH THE TABLES
SELECT E.EMP_NAME,D.DEPT_NAME FROM EMPLOYEE E INNER JOIN DEPT D ON E.DEPT_ID = D.DEPT_ID;

-- 2. LEFT JOIN - ALL THE RECORDS FROM THE LEFT TABLE AND MATCHED RECORDS FROM RIGTH TABLE
SELECT E.EMP_NAME,D.DEPT_NAME FROM EMPLOYEE E LEFT JOIN DEPT D ON E.DEPT_ID = D.DEPT_ID;

-- 3. RIGHT JOIN - ALL THE RECORDS FROM THE RIGHT TABLE AND MATCHED RECORDS FROM THE LEFT TABLE
...
-- 5. CROSS JOIN - FROM LEFT TABLE EVERY ROW IS PAIRED WITH EVERY ROW FROM RIGHT TABLE (INTERVIEW PURPOSE ONLY)
SELECT E.EMP_NAME,D.DEPT_NAME FROM EMPLOYEE E CROSS JOIN DEPT D ;

-- SELF JOIN - A TABLE CAN JOIN ITSELF
-- ADD A COLUMN IN EMPLOYEE TABLE ADD THE MANAGER ID
ALTER TABLE EMPLOYEE ADD MANAGER_ID INT;
SET SQL_SAFE_UPDATES= 1;
UPDATE EMPLOYEE SET MANAGER_ID = NULL WHERE EMP_ID =1;
UPDATE EMPLOYEE SET MANAGER_ID = 1 WHERE EMP_ID IN (2,3);
UPDATE EMPLOYEE SET MANAGER_ID = 2 WHERE EMP_ID = 4;
SELECT E.EMP_NAME AS EMP , M.EMP_NAME AS MAN 
...
-- THESE PERFORM CALCULATIONS ACROSS A SET OF TABLE ROWS 
-- WHY ARE WE USING WINDOW FUNCTIONS
-- PERFORMING RUNING TOTAL , RANKING , PERCENTILE , AVG ETC
-- WE CAN ANALYZE THE ROWS WHILE KEEPING THE FULL DETAILS OF THE TABLE

-- CREATE A TABLE ON SALES
CREATE TABLE SALES(ID INT PRIMARY KEY, EMP_NAME VARCHAR(25), DEPT VARCHAR(50),SALE_AMT DECIMAL(10,2), SALE_DATE DATE);

-- INSERT VALUES INTO THE TABLE

INSERT INTO SALES VALUES(1,'ANDERSON','HR',1500,'2025-01-01'),(2,'BENNY','IT',1000,'2025-01-02'),(3,'ANDERSON','HR',1200,'2025-01-03'),(4,'BENNY','IT',1700,'2025-01-04'),
(5,'CHARLIE','IT',800,'2025-01-04'),(6,'ANDERSON','HR',900,'2025-01-06');

-- VIEW THE SALES TABLE/ CHECK THE DATA OF SALES
SELECT * FROM SALES;

-- SYNTAX - FUNCTION_NAME() OVER(PARTITION BY COLUMN ORDER BY COLUMN) FROM TABLE_NAME

-- 1. ROW_NUMBER -- ROW NUMBERING
-- GETS THE NUMBERS OF ROWS FOR SALES TABLE BASED ON DATE AND GET THE RECORDS OF SALES AMOUNT
SELECT EMP_NAME,SALE_AMT, ROW_NUMBER() OVER(PARTITION BY DEPT ORDER BY SALE_DATE) AS SALE_AMT_DATE FROM SALES;
-- 2. RANK() AND DENSE_RANK() - RANKING THE ROWS
-- RANK THE ROWS ACCORDING TO THE SALE AMOUNT
SELECT EMP_NAME,SALE_AMT, RANK()
OVER(PARTITION BY EMP_NAME ORDER BY SALE_AMT DESC) AS RANK_ROW FROM SALES;

-- DIFFERENCE BETWEEN RANK AND DENSE RANK
-- IN RANK() IT SKIPS THE NUMBER IF THERE IS TIE
-- IN DENSE_RANK() IT DOESNT SKIP THE NUMBER ON TIE
INSERT INTO SALES VALUES(7,'ANDERSON','HR',1200,'2025-01-07');

SELECT EMP_NAME,SALE_AMT, DENSE_RANK()
OVER(PARTITION BY EMP_NAME ORDER BY SALE_AMT DESC) AS RANK_ROW FROM SALES;

-- 3. SUM() - RUNNING TOTAL - IS NOTHING BUT YOUR CUMULATIVE FREQUENCY.
SELECT EMP_NAME,SALE_AMT,SUM(SALE_AMT) OVER(PARTITION BY EMP_NAME ORDER BY SALE_DATE) AS RUNNING_TOTAL FROM SALES;
	
-- 4. AVG() - IT CAN BE MOVING OR GROUPED AVG
SELECT EMP_NAME,SALE_AMT,AVG(SALE_AMT) OVER(PARTITION BY EMP_NAME ORDER BY SALE_DATE) AS RUNNING_AVG FROM SALES;

-- 5. LEAD() AND LAG() - COMPARE THE NEXT SALE AND PAST ROW SALE
SELECT EMP_NAME,SALE_DATE,SALE_AMT,LEAD(SALE_AMT) OVER(PARTITION BY EMP_NAME ORDER BY SALE_DATE) AS LEAD_DATA FROM SALES;
SELECT EMP_NAME,SALE_DATE,SALE_AMT,LAG(SALE_AMT) OVER(PARTITION BY EMP_NAME ORDER BY SALE_DATE) AS LAG_DATA FROM SALES;

-- 6. NTILE(n) - DIVIDES THE ROWS INTO N GROUPS
SELECT EMP_NAME,SALE_DATE,SALE_AMT, NTILE(3)
OVER(PARTITION BY EMP_NAME ORDER BY SALE_AMT) AS BUCKET FROM SALES;
...
-- ########################## 08-06-2025 ###################################
-- WHAT IS VIEWS ?
-- IT IS A VIRTUAL TABLE BASED ON RESULT OF SELECT QUERY
-- IT DOES NOT STORE THE DATA LIKE TABLES
-- IT IS LIKE A VIRTUAL REPRENETATION OF YOUR TABLE

SELECT * FROM CUSTOMER;
-- CREATE A VIEW ON DOB OF 1965-01-01
CREATE VIEW ACTIVE_CUSTOMER AS SELECT CUSTOMER_ID,LAST_NAME,PHONE FROM CUSTOMER WHERE DOB IN ('1965-01-01');

-- CHECK THE DATA FROM ACTIVE_CUSTOMER
SELECT * FROM ACTIVE_CUSTOMER;

-- UPDATE THE NAME TO GREEN FROM VIEW AND IT CAN BE IMPACT ON THE ORIGINAL TABLE
SET SQL_SAFE_UPDATES =0;
UPDATE ACTIVE_CUSTOMER SET LAST_NAME = 'GREEN' WHERE CUSTOMER_ID = 1;
-- ####### INDEX #######
-- IT IS USED TO FASTER RETRIVEL OF THE DATA 'SELECT'
-- WHEN WE USE INDEX IT SLOW DOWN THE OPERATION OF INSERT DELETE AND UPDATE

SELECT * FROM PRODUCTS;
-- 1. SIMPLE INDEX - CREATING INDEX ONLY ON ONE COLUMN
-- CREATE AN INDEX ON COLUMN CALL 'NAME' FROM PRODUCT TABLE
-- GET THE RECORD OF PRODUCT NAME ZFILE
...
-- 2. COMPOSITE INDEX -- CREATING INDEX ON MULTIPLE COLUMNS
CREATE INDEX PRODUCT_IDX ON PRODUCTS(PRODUCT_TYPE_ID,NAME);
SELECT * FROM PRODUCTS WHERE PRODUCT_ID = 2 OR NAME = 'CHEMISTRY';
EXPLAIN SELECT * FROM PRODUCTS WHERE PRODUCT_ID = 2 OR NAME = 'CHEMISTRY';

-- 3 UNIQUE INDEX - IT IS CREATED WHEN THERE IS NO DUPLICATE DATA IN THE COLUMNS
...
-- WE CANNOT DROP MULTIPLE INDEXES AT A TIME WE NEED TO GO STEP BY STEP
